...
Use `sample-content/` as the committed public sample source. The Astro mount points under `src/content/blog/` and `public/` are generated by the sync step and should not be the canonical source for sample content.

Local builds may fall back to `sample-content/` when private content is missing. Real publishing must use strict sync (`PRIVATE_CONTENT_STRICT=1` or `npm run sync:content:strict`) so missing private blog posts fail before deployment.
